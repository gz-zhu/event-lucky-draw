...
- 参加者リストと下書きは同じブラウザに自動保存されるため、リロード・誤って閉じた場合・停電後でも、同じ端末と同じブラウザなら復元できます。
- 名前リストの下には、入力数・抽選対象数・重複数・当選済み除外数がリアルタイム表示されます。
- カウントダウンは1回ごとの設定です。Auto Draw でタイムアップ後に抽選が走ると、そのカウントダウン設定は自動でクリアされます。
- カウントダウン実行中は、他の賞ボタンと Settings がロックされ、現場での誤操作を防ぎます。
- 抽選時間は Settings で `5秒` 刻みで調整できます。
...
保存前に、リスト下の件数表示で実際の抽選対象人数を確認できます。
...
カウントダウン開始後は、終了またはキャンセルまで他の賞や設定操作はロックされます。
