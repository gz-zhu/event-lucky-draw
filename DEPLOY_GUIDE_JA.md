...
- 🔥 Firebase リアルタイム同期 — 全デバイスへ即時反映
- 📺 大画面表示ページ (`/display.html`) — プロジェクター向け、リアルタイムで当選者・統計・カウントダウンを表示
- 🔢 毎回の抽選で抽選シードを表示（監査対応）
...
### 3. （任意）プロジェクターに大画面表示を映す
ページ上部の **📺 Display** リンクをクリックして `display.html` を新しいタブで開く。
このタブをプロジェクターやサブスクリーンに映すと、リアルタイムで以下を表示します：当選者一覧、次の賞、参加者統計、時計。
